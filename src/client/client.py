...
# Current version: Ryan Song [ryan.song@epfl.ch]
# ############################################################################
...
if __name__ == '__main__':
    ...
    if N_sample > 500:
        raise ValueError(('Parameter[input_file] contains more than 500 samples. '
                          'Design a more efficient communication system.'))

    energy = np.sum(np.square(np.abs(tx_p_signal)))
    if energy > 1200:
        raise ValueError(('Energy of the signal exceeds the limit 1,200. '
                          'Design a more efficient communication system.'))
