...
def plot_constellations(result):
    ...
    WINDOW_AMPLITUDE = 1 + src.config.BOOST_FACTOR
    ...
    # Puncture modulated signal so that it match the size of the transmitted and recevied signals
    signal_in = src.utils.puncture(signal_in, src.config.PUNCTURED_BIT, src.config.K)
    in_re     = signal_in[0::2]
    in_im     = signal_in[1::2]
    n_symbols = len(in_re)
    ...
    ax1.scatter(raw_re, raw_im,
                c=symbol_colors, alpha=0.5, s=15, zorder=2)
    ...
    ax1.set_xlim([-d * WINDOW_AMPLITUDE, d * WINDOW_AMPLITUDE])
    ax1.set_ylim([-d * WINDOW_AMPLITUDE, d * WINDOW_AMPLITUDE])
    ...
    ax2.set_xlim([-d * WINDOW_AMPLITUDE, d * WINDOW_AMPLITUDE])
    ax2.set_ylim([-d * WINDOW_AMPLITUDE, d * WINDOW_AMPLITUDE])
